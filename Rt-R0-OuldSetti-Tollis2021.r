...
##They suggest the Durbin-Watson test which sounds very reasonable (you compute D=sum((e(t)-e(t-1))^2),
##all timepoints from timepoint 2)/sum((e(t))^2),all timepoints from timepoint 1).
transit.de.D <- sum(
  (transit.de.resid-lag(transit.de.resid,1))^2 #, ...
  )
##This D number should be between 0 and 4, and around 2 if no autocorrels.
# Then you compare the value of the coef D to tabulated values, for the significance of the autocorrelations, taking into account the sample size (number of timepoints). (https://online.stat.psu.edu/stat501/sites/stat501/files/14outliers/durbinwatson.pdf)
# If autocorrelations are significant and strong then we need to correct for (and this is quite complex).
...
#### I used durb for residential to correct the autocorr in Rt
...
#### 10. Correlation with moving window ####
movingcor <- data.frame("country"=rep(unique(mobil5$country),each=21),"frame"=1:21, 
                        "start" = seq(from=as.Date("2020-02-20"), to = as.Date("2020-11-30"), by=14),
                        "end" = c(
                          (seq(from=as.Date("2020-02-20"), to = as.Date("2020-11-30"), by=14)+(6*7))[-21],
                        as.Date("2021-01-01")),
                        "cor0"=NA,"cor14"=NA,"delta.cum.ratio"=NA)

for (i in unique(mobil5$country)) {
  for (k in 1:21) {
    tryCatch({
    movingcor[movingcor$country==i,]$cor0[k] <-
      cor(mobil5$residential[mobil5$country == i &
                               (mobil5$date >= movingcor[movingcor$country ==
                                                           i & movingcor$frame == k, "start"] &
                                  mobil5$date < movingcor[movingcor$country ==
                                                            i & movingcor$frame == k, "end"])],
          mobil5$R[mobil5$country == i &
                     (mobil5$date >= movingcor[movingcor$country == i &
                                                 movingcor$frame == k, "start"] &
                        mobil5$date < movingcor[movingcor$country ==
                                                  i & movingcor$frame == k, "end"])],
          use = "complete.obs")
    
    movingcor[movingcor$country == i, ]$cor14[k] <-
      ifelse(k %notin% c(1,21),
             cor(mobil5$residential[mobil5$country == i &
                                          (mobil5$date >= movingcor[movingcor$country ==
                                                                      i & movingcor$frame == k-1, "start"] &
                                             mobil5$date < movingcor[movingcor$country ==
                                                                       i & movingcor$frame == k-1, "end"])],
                 mobil5$R[mobil5$country == i &
                            (mobil5$date >= movingcor[movingcor$country == i &
                                                        movingcor$frame == k, "start"] &
                               mobil5$date < movingcor[movingcor$country == i &
                                                         movingcor$frame == k, "end"])], use = "complete.obs"),
             NA)
    
    movingcor[movingcor$country==i,]$delta.cum.ratio[k] <-
      (mobil5$cumcases[mobil5$country == i & 
                        mobil5$date == movingcor$end[movingcor$country == i & movingcor$frame == k]] - 
      mobil5$cumcases[mobil5$country == i & 
                        mobil5$date == movingcor$start[movingcor$country == i & movingcor$frame == k]]) /
      mobil5$cumcases[mobil5$country==i & mobil5$date == "2020-12-31"]
...
pdf(file="Export/movingcor.pdf") 
for (i in unique(movingcor$country)){
  plot(cor0~frame,data=movingcor[movingcor$country==i,],ty="l",
       main=paste0("Pearson's correlation by window - ",i),
       ylim=c(-1,1),xlab="6-week time windows (shifted eacg 2 weeks)",
       ylab="Pearson r (Rt ~ residential mobility)",axes=FALSE)
  axis(side = 1, at = 1:21)
  axis(side = 2, at = seq(-1,1,by=0.4))
  abline(h=0,lty=3)
  lines(cor14~frame,data=movingcor[movingcor$country==i,],ty="l",col=2)
  lines(I(rescale(newcases,to=c(0,1)))~I(rescale(date,to=c(1,21))),data=mobil5[mobil5$country==i,],
        ty="l",col=4,lty=2)
  legend("bottomright",legend = c("Correlation coef. with no lag","Correlation coef. with 14 days of lag",
                                  "Normalized daily case count (no windows considered)"),
         lty=c(1,1,2), col=c(1,2,4),cex=0.7)
}
dev.off() 

pdf(file="Export/movingcor-cumcases.pdf") 
for (i in unique(movingcor$country)){
  plot(delta.cum.ratio~frame,data=movingcor[movingcor$country==i,],ty="l",
       main=paste0("Cumulated cases slope by window - ",i),
       ylim=c(0,1),xlab="6-week time windows (shifted each 2 weeks)",
       ylab="Cumulated cases slope (%)",axes=FALSE)
  axis(side = 1, at = 1:21)
  axis(side = 2, at = seq(0,1,by=0.2))
  lines(I(rescale(newcases,to=c(0,1)))~I(rescale(date,to=c(1,21))),data=mobil5[mobil5$country==i,],
        ty="l",col=1,lty=3)
  legend("topleft",legend = c("Cumulated cases slope by window",
                              "Normalized daily case count (no windows considered)"),
         lty=c(1,3), col=c(1,1),cex=0.7)
}
dev.off() 
# 
# pdf(file="Export/movingcor-heat.pdf") 
# for (i in unique(movingcor$country)){
#   ggplot(movingcor[movingcor$country == i,], aes(cor0,  delta.cum.ratio, fill = frame)) + 
#     #geom_point()
#     geom_tile()
# }
# dev.off() 

movingcor.nona <- na.omit(movingcor)
hist(movingcor.nona$delta.cum.ratio,movingcor.nona$cor0)
#movingcor.nona$slopes <- cut(movingcor.nona$delta.cum.ratio,breaks = 0:1)
#hist(movingcor.nona$slopes,movingcor.nona$cor0)
#barplot(movingcor.nona$slopes, movingcor.nona$cor0)

movingcor.acceleration <- movingcor.nona[movingcor.nona$delta.cum.ratio > 0.15 & movingcor.nona$delta.cum.ratio < 0.25,
                               c("cor0","delta.cum.ratio")]
movingcor.deceleration <- movingcor.nona[movingcor.nona$delta.cum.ratio <= 0.15 | movingcor.nona$delta.cum.ratio >= 0.25,
                                         c("cor0","delta.cum.ratio")]


postscript("Export/fig3.eps", width=6,height=6,
           horizontal = FALSE, onefile = FALSE, paper = "special")

hist(movingcor.deceleration$cor0, ylim = c(0,600), xlim = c(-1,1), breaks = 20, col=rgb(0,0,1,0.3),
     xlab = "Rt vs residential Pearson's r coef.", ylab = "Occurence (number of periods)",
     main = "Histogram")
hist(movingcor.acceleration$cor0, ylim = c(0,600), xlim = c(-1,1), breaks = 20, col=rgb(1,0.4,0.4,1), add = TRUE)
legend("topright",legend = c("15% < Cases slope < 25%",
                             "Cases slope ≤ 15% or ≥ 25%"),
       col=c(rgb(1,0.4,0.4,1),rgb(0,0,1,0.3)),pch = 15, cex = 0.7)

dev.off()


tiff("Export/fig3.tiff", units="in", width=6,height=6,res=300) 
...
#and that would be figure 3

#### 11. Grouping the countries ####
...
#### 12. Scatter plots of residential vs other mobility indicators ####
...
#### 13. Scatter plots of transit vs mobility ####
...
#### 14. Plots for p-values of corlag ####
